...
template<typename T, template<typename U> class Descriptor>
class SetDensity : public BoxProcessingFunctional2D_L<T,Descriptor> {
public:
    SetDensity(plint dx_, plint dy_)
        : dx(dx_), dy(dy_)
    { }
    ...
    virtual void process(Box2D domain, BlockLattice2D<T,Descriptor>& lattice) {
        ...
        for (plint iX=domain.x0; iX<=domain.x1; ++iX){
            for (plint iY=domain.y0; iY<=domain.y1; ++iY){
                rho = lattice.get(iX+dx, iY+dy).computeDensity();
...
void writeVTK(  MultiBlockLattice2D<T,DESCRIPTOR>& leftFluid,
                MultiBlockLattice2D<T,DESCRIPTOR>& rightFluid,
                T dx, T dt, plint iter)
{
    ...
    vtkOut.writeData<2,float>(*computeVelocity(leftFluid), "velocityleft", dx/dt);
    vtkOut.writeData<float>(*computeDensity(rightFluid), "densityright", 1.);
    vtkOut.writeData<2,float>(*computeVelocity(rightFluid), "velocityright", dx/dt);
}
...
int main(int argc, char *argv[])
{
    ...
    MultiBlockLattice2D<T, DESCRIPTOR> rightFluid (
            nx,ny, new ExternalMomentRegularizedBGKdynamics<T, DESCRIPTOR>(omega) );
    MultiBlockLattice2D<T, DESCRIPTOR> leftFluid (
            nx,ny, new ExternalMomentRegularizedBGKdynamics<T, DESCRIPTOR>(omega) );
    ...
    integrateProcessingFunctional (
        ...
        Box2D RightWall(nx-1,nx-1, 0, ny-1);
        Box2D outletDomain(1,nx-2, 0,0);
        Box2D inlet1Domain(1,inletSize, ny-1,ny-1);
        Box2D inlet2Domain(inletSize+1, nx-2, ny-1, ny-1);

        pcout << "nx: " << nx << ", ny: " << ny << std::endl;

        // Bounce-back on bottom wall.
        defineDynamics(leftFluid, outletDomain, new VelocityBounceBack<T, DESCRIPTOR>((T)1., Array<T,2>((T) 0., -uAveLB)) );
        defineDynamics(rightFluid, outletDomain, new VelocityBounceBack<T, DESCRIPTOR>((T)1., Array<T,2>((T) 0., -uAveLB)) );
        // Bounce-back on top wall.
            // On the left inlet
        defineDynamics(leftFluid, inlet1Domain, new VelocityBounceBack<T, DESCRIPTOR>((T)1, Array<T,2>((T) 0., -uAveLB)) );
        defineDynamics(rightFluid, inlet1Domain, new VelocityBounceBack<T, DESCRIPTOR>((T)1.e-4, Array<T,2>((T) 0., -uAveLB)) );
            // On the right inlet
        defineDynamics(leftFluid, inlet2Domain, new VelocityBounceBack<T, DESCRIPTOR>((T)1.e-4, Array<T,2>((T) 0., -uAveLB)) );
        defineDynamics(rightFluid, inlet2Domain, new VelocityBounceBack<T, DESCRIPTOR>((T)1, Array<T,2>((T) 0., -uAveLB)) );
        // Define Bounceback left wall
        defineDynamics(leftFluid, LeftWall, new VelocityBounceBack<T, DESCRIPTOR>((T)1., Array<T,2>((T) 0., (T) 0.)) );
        defineDynamics(rightFluid, LeftWall, new VelocityBounceBack<T, DESCRIPTOR>((T)1.e-4, Array<T,2>((T) 0., (T) 0.)) );
        // Define Bounceback right wall
        defineDynamics(leftFluid, RightWall, new VelocityBounceBack<T, DESCRIPTOR>((T)1.e-4, Array<T,2>((T) 0., (T) 0.)) );
        defineDynamics(rightFluid, RightWall, new VelocityBounceBack<T, DESCRIPTOR>((T)1., Array<T,2>((T) 0., (T) 0.)) );


        // Initialize top layer.
        applyIndexed(rightFluid, Box2D(0, nx-1, 0, ny-1),
                     new TwoLayerInitializer<T,DESCRIPTOR>(nx, true) );
        // Initialize bottom layer.
        applyIndexed(leftFluid, Box2D(0, nx-1, 0, ny-1),
    ...
    while(!velocityTracer1.hasConverged() || !velocityTracer2.hasConverged())
    {
        ...
        applyProcessingFunctional(new SetDensity<T,DESCRIPTOR>(1,0), LeftWall, rightFluid);
        applyProcessingFunctional(new SetDensity<T,DESCRIPTOR>(1,0), LeftWall, leftFluid);
        applyProcessingFunctional(new SetDensity<T,DESCRIPTOR>(-1,0), RightWall, rightFluid);
        applyProcessingFunctional(new SetDensity<T,DESCRIPTOR>(-1,0), RightWall, leftFluid);
        applyProcessingFunctional(new SetDensity<T,DESCRIPTOR>(0,1), outletDomain, rightFluid);
        applyProcessingFunctional(new SetDensity<T,DESCRIPTOR>(0,1), outletDomain, leftFluid);
